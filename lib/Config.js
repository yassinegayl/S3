...
class Config {
    ...
    _getConfig() {
        ...
        if (config.dataDaemon) {
            if (config.dataDaemon.host !== undefined) {
                assert.strictEqual(typeof config.dataDaemon.host, 'string',
                                   'bad config: data daemon host must be ' +
                                   'a string');
                this.dataDaemon.host = config.dataDaemon.host;
            }
            if (config.dataDaemon.port !== undefined) {
                assert(Number.isInteger(config.dataDaemon.port)
                       && config.dataDaemon.port > 0,
                       'bad config: dataDaemon port must be a positive ' +
                       'integer');
            ...
            this.dataDaemon.dataPath = dataPath;
        }
