...
function assertCertPaths(key, cert, ca, basePath) {
    ...
    let certpath;
    if (key && cert) {
        keypath = (key[0] === '/') ? key : `${basePath}/${key}`;
        certpath = (cert[0] === '/') ?
            cert : `${basePath}/${cert}`;
        assert.doesNotThrow(() =>
        ...
        certPathsObj.keypath = keypath;
        certPathsObj.certpath = certpath;
    }

    let capath;
    if (ca) {
        capath = (ca[0] === '/') ? ca : `${basePath}/${ca}`;
        assert.doesNotThrow(() =>
            fs.accessSync(capath, fs.F_OK | fs.R_OK),
            `File not found or unreachable: ${capath}`);
        certPathsObj.capath = capath;
    }
    return certPathsObj;
}
...
class Config extends EventEmitter {
    ...
    _getLocationConfig() {
        ...
        Object.keys(locationConfig).forEach(l => {
            ...
            const d = locationConfig[l].details;
            if (d.connector !== undefined) {
                assert(typeof d.connector === 'object',
                    'bad config: connector must be an object');
                if (d.connector.sproxyd !== undefined) {
                    details.connector.sproxyd = d.connector.sproxyd;
                    const fields = sproxydAssert(d.connector.sproxyd);
                    if (fields.indexOf('bootstrap') > -1) {
                        details.connector.sproxyd.bootstrap =
                            d.connector.sproxyd.bootstrap;
                    ...
                    if (fields.indexOf('chordCos') > -1) {
                        details.connector.sproxyd.chordCos =
                            cosParse(d.connector.sproxyd.chordCos);
            ...
            if (d.certFilePaths !== undefined) {
                assert(typeof d.certFilePaths === 'object', 'bad config: ' +
                    'details.certFilePaths should be an object');
                if (d.certFilePaths.key !== undefined ||
                    d.certFilePaths.cert !== undefined) {
                    assert(d.certFilePaths.key && d.certFilePaths.cert
                    && typeof d.certFilePaths.key === 'string' &&
                    typeof d.certFilePaths.cert === 'string',
                    'bad config: if either details.certFilePaths.key or ' +
                    'details.certFilePaths.cert is defined, the other must ' +
                    'also be defined and they must both be strings');
                }
                if (d.certFilePaths.ca !== undefined) {
                    assert(typeof d.certFilePaths.ca === 'string',
                    'bad config: details.certFilePaths.ca must be a string');
                }
                const { key, cert, ca } = d.certFilePaths;
                const certPathsObj =
                    assertCertPaths(key, cert, ca, this._basePath);
                if (Object.keys(certPathsObj.length > 0)) {
                    details.certPaths = certPathsObj;
                }
            }
        });
    }
    ...
    _getConfig() {
        ...
        let certPathsObj = undefined;
        if (key && cert) {
            certPathsObj = assertCertPaths(key, cert, ca, this._basePath);
        } else if (key || cert) {
            throw new Error('bad config: both certFilePaths.key and ' +
                'certFilePaths.cert must be defined');
        }
        if (certPathsObj && Object.keys(certPathsObj).length > 0) {
            this.https = {
                cert: cert ? fs.readFileSync(certPathsObj.certpath, 'ascii') :
                    undefined,
                key: key ? fs.readFileSync(certPathsObj.keypath, 'ascii') :
                    undefined,
                ca: ca ? fs.readFileSync(certPathsObj.capath, 'ascii') :
                    undefined,
            };
            this.httpsPath = {
                ca: certPathsObj.capath,
                cert: certPathsObj.certpath,
            };
        }
