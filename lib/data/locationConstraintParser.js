    ...
    Object.keys(config.locationConstraints).forEach(location => {
        ...
        if (locationObj.type === 'aws_s3') {
            if (process.env.CI_PROXY === 'true') {
                ...
                locationObj.details.certFilePaths.key = '/Users/dorakorpar/squid.key';
                locationObj.details.certFilePaths.cert = '/Users/dorakorpar/squid.crt';
            }
            const connectionAgent = locationObj.details.https ?
                new https.Agent({ keepAlive: true, timeout: 10, retries: 0 }) :
                new http.Agent({ keepAlive: true, timeout: 10, retries: 0 });
