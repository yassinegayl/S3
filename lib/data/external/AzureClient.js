...
class AzureClient {
    constructor(config) {
        ...
        if (config.proxy) {
            ...
            if (config.certPaths) {
                const { keypath, certpath, capath } = config.certPaths;
                proxyParams.cert = certpath ?
                    fs.readFileSync(certpath, 'ascii') : undefined;
                proxyParams.key = keypath ?
                    fs.readFileSync(keypath, 'ascii') : undefined;
                proxyParams.ca = capath ?
                    fs.readFileSync(capath, 'ascii') : undefined;
            }
