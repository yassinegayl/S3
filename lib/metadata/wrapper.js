...
const metadata = {
    ...
    getAllObjects: (bucketName, log, cb) => {
        log.debug('getting all objects for bucket from metadata');
        client.getAllObjects(bucketName, log, (err, stream) => {
            if (err) {
                log.debug('error from metadata', { implName, err });
                return cb(err);
            }
            log.debug('all objects for a bucket retreived from metadata');
