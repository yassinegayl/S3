...
const { describeSkipIfNotMultiple, getBkndRetry,
...
function getSuccess(key, awsMD5, location, s3Type, cb) {
    return getBkndRetry({ key, s3Type }, 0, (err, res) => {
...
function getError(key, expectedError, s3Type, cb) {
    return getBkndRetry({ key, s3Type }, 0, err => {
...
function getCheck(objectKey, s3MD5, awsMD5, location, s3Type, cb) {
    ...
    function s3GetCallback(err, res) {
        ...
        return getSuccess(objectKey, awsMD5, location, s3Type, cb);
    });
}
        ...
        'location-constraint" header', withGCP(s3Type => {
            ...
            it(`should put a 0-byte object to ${s3Type}`, done => {
                ...
                return s3.putObject(params, err => {
                    assert.equal(err, null, 'Expected success, ' +
                    `got error ${err}`);
                    return getCheck(key, emptyMD5, emptyMD5, s3Location,
                        s3Type, () => done());
                });
            });
            ...
            it(`should put an object to ${s3Type}`, done => {
                ...
                return s3.putObject(params, err => {
                    assert.equal(err, null, 'Expected success, ' +
                        `got error ${err}`);
                    return getCheck(key, correctMD5, correctMD5, s3Location,
                        s3Type, () => done());
                });
            });
            ...
            `object to ${s3Type}`,
                ...
                return s3.putObject(params, err => {
                    assert.equal(err, null, 'Expected success, ' +
                        `got error ${err}`);
                    return getSuccess(key, correctMD5, s3Location,
                        s3Type, () => done());
                });
            });

            skipIfGCP(`should put an object to ${s3Type} with encryption`,
            done => {
                const key = `somekey-${Date.now()}`;
                const params = { Bucket: bucket, Key: key,
                    Body: body,
                ...
                return s3.putObject(params, err => {
                    assert.equal(err, null, 'Expected success, ' +
                        `got error ${err}`);
                    return getCheck(key, correctMD5, correctMD5,
            ...
            `to ${s3Type} with versioning enabled`, done => {
                ...
                async.waterfall([
                    ...
                    (eTag, next) => getSuccess(key, correctMD5, s3Location,
                        s3Type, () => next()),
                ], done);
            });

            it(`should put a large object to ${s3Type}`, done => {
                const key = `somekey-${Date.now()}`;
                const params = { Bucket: bucket, Key: key,
                    Body: bigBody,
                    Metadata: { 'scal-location-constraint': s3Location } };
                return s3.putObject(params, err => {
                    assert.equal(err, null, 'Expected sucess, ' +
                        `got error ${err}`);
                    return getCheck(key, bigS3MD5, bigAWSMD5, s3Location,
                        s3Type, () => done());
                });
            });
                    ...
                    return s3.putObject(params, err => {
                        ...
                        (err, res) => {
                            ...
                            assert.strictEqual(
                                ...
                                fileLocation);
                            return getError(key, 'NoSuchKey', s3Type, done);
                        });
                    });
                });
            });
        ...
        it(`should put an object to ${s3Type} with no location header`,
            ...
            return s3.createBucket({ Bucket: bucket,
                ...
                return s3.putObject(params, err => {
                    ...
                    return getCheck(key, correctMD5, correctMD5, undefined,
                        s3Type, () => done());
                });
            });
        });
